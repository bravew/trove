...
description: "Migrate Callers Then Delete Legacy APIs — when introducing a new internal API while old callers exist, migrate every caller and delete the old API in the same refactor wave instead of leaving a compatibility layer. Use when replacing an internal API, function, or module that has no external dependents. Use when: migrate callers; remove the legacy api; no compatibility shim."
---
