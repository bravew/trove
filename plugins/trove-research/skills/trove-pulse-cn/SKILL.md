...
allowed-tools:
  ...
  - WebSearch
metadata:
  source: Jesseovo/last30days-skill-cn
  upstream-version: 3.2.0-cn
---
...
```bash
python3 ${CLAUDE_SKILL_DIR}/scripts/last30days.py "<topic>" --emit compact
```

Useful variants:

```bash
python3 ${CLAUDE_SKILL_DIR}/scripts/last30days.py "<topic>" --quick --emit compact
python3 ${CLAUDE_SKILL_DIR}/scripts/last30days.py "<topic>" --deep --emit md
python3 ${CLAUDE_SKILL_DIR}/scripts/last30days.py "<topic>" --emit html-path
python3 ${CLAUDE_SKILL_DIR}/scripts/last30days.py "<topic>" --search weibo,bilibili,zhihu --emit compact
python3 ${CLAUDE_SKILL_DIR}/scripts/last30days.py "<topic>" --as-of 2026-05-01 --emit compact
python3 ${CLAUDE_SKILL_DIR}/scripts/last30days.py "<topic>" --refresh --emit compact
python3 ${CLAUDE_SKILL_DIR}/scripts/last30days.py "<topic>" --no-cache --emit compact
python3 ${CLAUDE_SKILL_DIR}/scripts/last30days.py --diagnose
python3 ${CLAUDE_SKILL_DIR}/scripts/last30days.py --diagnose --emit json
python3 ${CLAUDE_SKILL_DIR}/scripts/last30days.py setup
```
