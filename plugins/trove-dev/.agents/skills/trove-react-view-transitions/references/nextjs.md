...
**Availability:** `transitionTypes` requires `experimental.viewTransition: true` and shipped on stable in **Next.js 16.2** ([PR #90701](https://github.com/vercel/next.js/pull/90701)). The `experimental.viewTransition` flag itself is still flagged "experimental, not recommended for production" in the 16.2 docs — pin Next.js and re-verify on upgrade. If unavailable in your version, use `startTransition` + `addTransitionType` + `router.push()` (see Programmatic Navigation below). To check: `grep -r "transitionTypes" node_modules/next/dist/` — if no results, fall back to programmatic navigation.
