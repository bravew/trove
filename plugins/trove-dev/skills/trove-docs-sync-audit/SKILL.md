---
name: trove-docs-sync-audit
description: "Read-only documentation drift audit comparing what the docs claim against what the code does. Covers READMEs, setup guides, API and env docs, changelogs, examples, and generated docs. Use when asked whether docs are stale, missing, or inconsistent after a code change. This is not a code review."
license: MIT
when_to_use: "are the docs stale; do the docs match the code; documentation drift; what docs need updating"
user-invocable: true
metadata:
  source: github/awesome-copilot
  upstream-skill: docs-sync-audit
---
...
finding carries a `P0`-`P3` severity and a `path:line` you can open. Route to
`trove-test-gap-audit` for missing test coverage, `trove-review` for a code
review of the diff, and `trove-doc` when the drift is confirmed and the docs
need writing rather than auditing.
