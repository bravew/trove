---
name: trove-test-gap-audit
description: "Read-only audit for missing, weak, stale, or mis-scoped test coverage. Audits the whole repository when no scope is named, or one feature, PR, branch, route, service, or risky change when it is. Use when asked what tests are missing, whether coverage is enough, or what regression tests would prove a change safe. This is not a bug hunt and not a security review; use trove-tdd to write the test it recommends."
license: MIT
when_to_use: "what tests are missing; is our coverage enough; what regression tests should we add; audit test coverage"
user-invocable: true
metadata:
  source: github/awesome-copilot
  upstream-skill: test-gap-audit
---
...
This skill shares one report contract with the other Trove audit skills: every
finding carries a `P0`-`P3` severity and a `path:line` you can open. It reports
gaps; `trove-tdd` writes the failing test that closes one. Route to
`trove-docs-sync-audit` for documentation drift and `trove-review` for a code
review of the diff.
